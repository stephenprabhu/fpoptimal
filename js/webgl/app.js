...
var loadResourcesInParallel = function(resources, callback) {
  var results = {};
  var errors = {};
  var completed = 0;
  ...
  resources.forEach(resource => {
    ...
    switch (type) {
      case 'vs':
        loadTextResource(url, handler);
        break;
      case 'fs':
        loadTextResource(url, handler);
        break;
      case 'model':
        loadJSONResource(url, handler);
        break;
      case 'image':
        loadImage(url, handler);
        break;
      default:
...
var MoreLight = function() {
  ...
  this.Init = function() {
    var resources = [
      { type: 'vs', url: './js/webgl/shader.vs.glsl' },
      { type: 'fs', url: './js/webgl/shader.fs.glsl' },
      { type: 'model', url: './js/webgl/monkey.json' },
      { type: 'image', url: './js/webgl/color.png' }
    ];

    loadResourcesInParallel(resources, function(errors, results) {
      if (Object.keys(errors).length > 0) {
        console.error(errors);
      } else {
        _this.run(results.vs, results.fs, results.model, results.image);
      }
    });
  };
